;;Developed by AMRNVB Pethiyagoda, HMVS Herath, MFAR Fernando, NS Madushanka

breed [redplanes redplane]
breed [blueplanes blueplane]
...
redplanes-own [
  ...
  velocity  ;;Velocity of the plane, non-negative
  health  ;;Health of the plane. The plane dies when health < 0.
  startXcor  ;;X coordinate when the plane is created
  startYcor  ;;Y coordinate when the plane is created
  wins  ;;Number of wins
  degree  ;;Degree to rotate the plane
...
globals [
  planeSize
  StartHealth
  CollisionHealth  ;;Health damage from collisions with planes or walls
  MaxTurretRotation
  VplaneAcc  ;;Acceleration of plane
  VplaneDec  ;;Deceleration of plane
  VplaneMax  ;;Max speed of plane
...
to setup
  ...
  set VplaneMax 1 * VFactor
  set VplaneAcc 0.2 * VFactor
  set VplaneDec -0.4 * VFactor
  set planes nobody
...
to obs-create-plane [argColor argPos]
  let tr nobody

  if planes != nobody and count planes with [color = argColor] > 0 [
    ask planes with [color = argColor] [
      ask myTurret [die]
      die
    ]
  ]
  create-turtles 1 [
    ...
    if argColor = blue [
      set breed blueplanes
      set planeBlue self
    ]
    if argPos = 1 [setxy max-pxcor - planeSize max-pycor - planeSize]
    if argPos = 5 [setxy min-pxcor + planeSize min-pycor + planeSize]
    set shape "airplane"
    set name planeName
    set color argColor
    set size planeSize
    ...
    set startXcor xcor
    set startYcor ycor
    set planes (turtle-set planes self)
...
to plane-accelerate
  set velocity min list VplaneMax (velocity + VplaneAcc)
end

to plane-decelerate
  set velocity max list 0 (velocity + VplaneDec)
end

to plane-change-velocity
  if acceleration = -1 [plane-decelerate]
  if acceleration = 1 [plane-accelerate]
end
...
to plane-prepare
  if color = red [plane-prepare-red]
  if color = blue [plane-prepare-blue]
end
...
to bullet-check-collision
  let tks planes with [distance myself < (planeSize / 2)]
  if count tks > 0 [
    ask tks [
...
to plane-prepare-red
  let tks other planes with [distance myself < planeSize + 3]
  ifelse count tks > 0 [status-stop-turn-plane][status-forward[0.01]]
  ...
  ask myTurret [
    let enemys other planes with [distance myself > planeSize] ;with [distance myself < planeSize + 10];
    if(count enemys > 0)[
